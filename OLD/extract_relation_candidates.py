...
def _require_cluster_type_layer(doc: Any, cluster_type_layer: Any | None) -> Any:
    ...
    if require_cluster_typing_layer is None:
        raise ValueError(
            "cluster_type_layer was not provided and cluster_typing.cluster_typing_schema "
...
def _print_discard(
    *,
    ...
    print(f"[{subject}][{predicate}][{obj}] discarded because [{reason}]")
            ...
            if not candidate_properties:
                if print_discards:
                    _print_discard(
                        ...
                        predicate=candidate.predicate,
                        obj=candidate.target_canonical_name,
                        reason="no_type_pair_candidates",
                    )
                continue
