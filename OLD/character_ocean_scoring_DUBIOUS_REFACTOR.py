...
@dataclass(frozen=True)
class MentionScoringRunConfig:
    """
    Execution-level configuration for mention scoring.

    This separates two concerns that were previously overloaded into one
    ``chunk_size`` argument:

    checkpoint_size:
        Number of mention records handled by the outer Python loop before
        progress is printed and complete ScoredMentionRecord rows are emitted.
        This is not the Hugging Face model batch size.

    print_task_progress:
        If True, print per-task timings inside each checkpoint: one OCEAN trait
        task at a time, then optional OCEAN_weight.

    synchronize_cuda_for_timing:
        If True, synchronize CUDA before/after timed model calls so timings are
        meaningful on GPU.
    """

    checkpoint_size: int = 128
    print_task_progress: bool = True
    synchronize_cuda_for_timing: bool = True

    def __post_init__(self) -> None:
        if self.checkpoint_size <= 0:
            raise ValueError(
                f"checkpoint_size must be > 0, got {self.checkpoint_size}"
            )
...
def score_ocean_texts_task_major(
    texts: list[str],
    *,
    subject: str | None = None,
    config: OceanScoringConfig | None = None,
    trait_labels: dict[str, dict[str, str]] | None = None,
    run_config: MentionScoringRunConfig | None = None,
    print_progress: bool = False,
    progress_prefix: str = "",
) -> list[dict[str, float]]:
    """
    Score many texts for OCEAN using task-major scheduling.

    Instead of treating a checkpoint as an opaque call, this function runs one
    OCEAN task at a time across the full provided text block:

        openness for all texts
        conscientiousness for all texts
        extraversion for all texts
        agreeableness for all texts
        neuroticism for all texts

    Hugging Face batching is still controlled only by ``config.batch_size``.
    The number of texts passed here should be understood as an outer
    checkpoint/block size, not the model batch size.
    """
    ...
    config = config or OceanScoringConfig()
    trait_labels = trait_labels or OCEAN_LABELS
    run_config = run_config or MentionScoringRunConfig()

    classifier = load_zero_shot_classifier(config.model_name)
    hypothesis_template = _hypothesis_template(config, subject)

    results: list[dict[str, float]] = [
        {trait: config.collapse.neutral_score for trait in OCEAN_TRAITS}
        for _ in texts
    ]

    for trait, labels in trait_labels.items():
        if run_config.synchronize_cuda_for_timing:
        ...
        if print_progress:
            print(
                f"{progress_prefix}task={trait} | "
                f"texts={len(texts)} | "
                f"model_batch_size={config.batch_size} | "
...
def _resolve_run_config(
    *,
    run_config: MentionScoringRunConfig | None,
    checkpoint_size: int | None,
    chunk_size: int | None,
) -> MentionScoringRunConfig:
    """
    Resolve execution-level configuration.

    ``chunk_size`` is preserved as a backward-compatible alias for
    ``checkpoint_size``. New code should prefer checkpoint_size/run_config.
    """

    if run_config is not None:
        if checkpoint_size is not None or chunk_size is not None:
            raise ValueError(
                "Pass either run_config or checkpoint_size/chunk_size, not both."
            )
        return run_config

    if checkpoint_size is not None and chunk_size is not None:
        raise ValueError("Pass either checkpoint_size or chunk_size, not both.")

    resolved_checkpoint_size = checkpoint_size
    if resolved_checkpoint_size is None:
        resolved_checkpoint_size = chunk_size
    if resolved_checkpoint_size is None:
        resolved_checkpoint_size = MentionScoringRunConfig().checkpoint_size

    return MentionScoringRunConfig(checkpoint_size=resolved_checkpoint_size)
...
def score_mention_records(
    ...
    *,
    ...
    """
    Score mention records with checkpointed task-major scheduling.

    Execution order inside each checkpoint is now task-major:

        checkpoint records 0:N
            openness for all checkpoint texts
            conscientiousness for all checkpoint texts
            extraversion for all checkpoint texts
            agreeableness for all checkpoint texts
            neuroticism for all checkpoint texts
            OCEAN_weight for all checkpoint texts, if enabled
            assemble ScoredMentionRecord rows

    ``checkpoint_size`` controls the outer Python/checkpoint block size.
    ``scoring_config.batch_size`` controls the Hugging Face batch size for OCEAN
    trait model calls.
    ``weight_config.batch_size`` controls the Hugging Face batch size for
    OCEAN_weight model calls.

    ``chunk_size`` is kept only as a backward-compatible alias for
    ``checkpoint_size``. New code should pass checkpoint_size or run_config.
    """
    ...
    n_checkpoints = (n_records + run_config.checkpoint_size - 1) // run_config.checkpoint_size

    if print_progress:
        print("=" * 100)
        print("Mention-level OCEAN scoring — task-major checkpoints")
        print(f"records: {n_records}")
        print(f"subject: {subject!r}")
        print(f"device: {_device_name()}")
        print(f"checkpoint_size: {run_config.checkpoint_size}")
        print(f"checkpoints: {n_checkpoints}")
        print(f"OCEAN model_batch_size: {scoring_config.batch_size}")
        print(f"OCEAN_weight enabled: {weight_config.enabled}")
        if weight_config.enabled:
            print(f"OCEAN_weight model_batch_size: {weight_config.batch_size}")
        else:
    ...
    for checkpoint_index, start in enumerate(
        range(0, n_records, run_config.checkpoint_size),
        start=1,
    ):
        end = min(start + run_config.checkpoint_size, n_records)
        checkpoint_records = records[start:end]
        checkpoint_contexts = [record.context_text for record in checkpoint_records]

        checkpoint_start = time.perf_counter()

        if print_progress:
            print(
                f"[checkpoint {checkpoint_index}/{n_checkpoints}] "
                f"mentions={start}:{end} | "
                f"texts={len(checkpoint_contexts)}"
            )

        checkpoint_scores = score_ocean_texts_task_major(
            checkpoint_contexts,
            subject=subject,
            config=scoring_config,
            trait_labels=trait_labels,
            run_config=run_config,
            print_progress=print_progress and run_config.print_task_progress,
            progress_prefix="  ",
        )
        ...
        if weight_config.enabled:
            ...
            checkpoint_weight_scores = score_ocean_weight_texts(
                checkpoint_contexts,
                subject=subject,
            ...
            if print_progress and run_config.print_task_progress:
                print(
                    f"  task=OCEAN_weight | "
                    f"texts={len(checkpoint_contexts)} | "
                    f"model_batch_size={weight_config.batch_size} | "
                    f"time={weight_elapsed:.2f}s"
                )
        else:
            checkpoint_weight_scores = score_ocean_weight_texts(
                checkpoint_contexts,
        ...
        if len(checkpoint_scores) != len(checkpoint_records):
            raise RuntimeError(
                "Internal scoring error: OCEAN score count does not match record count."
            )

        if len(checkpoint_weight_scores) != len(checkpoint_records):
            raise RuntimeError(
        ...
        if print_progress:
            print(
                f"  checkpoint_time={checkpoint_elapsed:.2f}s | "
...
def benchmark_ocean_mention_records(
    ...
    *,
    ...
    Execution is task-major inside each checkpoint.

    Prefer ``checkpoint_size`` or ``run_config`` for the outer checkpoint size.
    ``chunk_size`` is kept as a backward-compatible alias. Hugging Face model
    batching is controlled separately by ``scoring_config.batch_size`` and
    ``weight_config.batch_size``.
    """
    ...
    df.attrs["benchmark"] = {
        ...
        "OCEAN_model_batch_size": scoring_config.batch_size,
        "OCEAN_weight_model_batch_size": weight_config.batch_size,
        "checkpoint_size": run_config.checkpoint_size,
        "legacy_chunk_size_argument": chunk_size,
        "device": _device_name(),
