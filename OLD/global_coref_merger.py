...
def sieve_exact_mention_overlap_coefficient(
    *,
    ...
    """Sieve 2: exact mention-key overlap coefficient.

    V1 accepts a pair when the smaller exact mention-key set is fully contained
    in the larger one and at least `min_shared_mentions` keys are shared.
    """
    sieve_name = "exact_mention_overlap_coefficient"

    candidate_pairs = unique_pairs_from_buckets(indexes.by_mention_key.values())
    accepted_edges: list[AcceptedMergeEdge] = []
    ...
    for left_uid, right_uid in candidate_pairs:
        ...
        shared_keys = left.mention_keys & right.mention_keys
        shared_count = len(shared_keys)

        if shared_count < min_shared_mentions:
            continue

        smaller_size = min(len(left.mention_keys), len(right.mention_keys))
        if smaller_size == 0:
            continue
        ...
        accepted_edges.append(
            AcceptedMergeEdge(
                ...
                metrics={
                    ...
                    "left_mentions": len(left.mention_keys),
                    "right_mentions": len(right.mention_keys),
